...
const SearchUser = () => {
    ...
    const Ebutton = () => {
        ...
        for (let i = 0; i < listSearch.length / 10; i++) {
            arrayBtn.push(<button key={i} className={`${i == abs && "active_Slide_Page"} btnPage`} onClick={() => setAbs(i)} >{i + 1}</button>)
        }
        return arrayBtn
    }
